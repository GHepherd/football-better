...
## 变更 3：复盘拆成「单场」与「方案」两个队列（2026-09-11）

**背景。** 变更 2 把方案定为独立产物，但没有相应地定独立**复盘单元**。原设计要求 reviewer 去「同目录」读 `plan-NN.md` 并把盈亏写进该场的 `-review.md`，于是有两个洞：

1. **跨比赛日的方案会漏评。** 方案按变更 2 落在本轮**最早**的比赛日目录，而 `SKILL.md` 与 reviewer manifest 都让 reviewer 只看「**同目录**」。一个横跨 2026-09-12 与 2026-09-13 的方案落在 09-12 目录，09-13 那场的复盘就完全看不到它。
2. **一份方案跨场时会被重复计账。** 方案骨架第 1 节是「比赛清单」（可多场），`-review.md` 却是**单场**文件。没人规定跨场方案的盈亏表归哪一场，两场各写一遍就把同一笔本金记了两次。

两个洞同一个根因：**方案被当成了单场复盘的附属品，而它其实是一个跨场的独立对象。**

**变更。**

- 方案复盘独立成产物 `matches/<该方案所在比赛日>/plan-NN-review.md`，就写在方案旁边。一份方案**只评一次、只写一个文件**。
- 单场 `-review.md` 删掉「投注方案」盈亏表，只留**一行不带金额**的指向。
- 待复盘判定拆成两个独立队列（见 `review-protocol.md` §一）。**方案队列的放行条件是「它覆盖的每一场都已有 `-review.md`」**—— 这就是「方案中有一场比赛没结束就先不复盘」的机械表达：未结束的比赛按 §三 不写 `-review.md`，于是方案自然不放行。方案因此**不需要自己的日期判定**，完全复用单场队列已产生的信息。
- 结算口径：按**方案写下时的竞彩赔率**，套**竞彩**玩法规则（让球胜平负用竞彩让球数，不是 analyzer 报的亚盘盘口）；混合过关须全中。从各场 `-review.md` 取比分，**不重新搜比分、不重算模型**。
- `lottery-strategist` 的「比赛清单」每条必须写成 `matches/<比赛日>/<slug>`，让放行条件可机械核对。

**为什么是「每一腿都结算」而不是「按日期猜」。** 混合过关在每一腿结算前**根本没有确定的回报**。所以这不是保守，是正确性：过早评分等于把一个错误的数字冻进记录。同一理由也解释了为什么未结束的比赛宁可不写 review —— 「没有结果」不能和「已复盘」混为一谈。

**未变更。** 单场复盘照旧（校准、归因、记忆修订本来就是单场的），方案复盘**不替代**它。第 4 步硬闸门不变。

**本变更与变更 2 的关系。** 变更 2 的「reviewer 对每一份 `plan-NN.md` 分别评分，不合并计算」由本变更**细化并取代**：仍是逐份评分，但评分位置从各场 `-review.md` 移到了 `plan-NN-review.md`，且多了「全场次结算」的放行条件。变更 2 的其余部分（落盘义务、`plan-NN` 命名、永不覆盖、观望也写文件）全部继续有效。

**本变更取代的规范正文。** 正文以下处仍按旧模型描述方案复盘，**以本变更为准**：§4 文件结构树、§5.3 复盘机制中的「投注方案（若存在 `plan.md`）」、§6 执行流程中的「Read summary.md → 写 plan.md」、§9 验收标准第 2/3/5 条。正文保留原文以存设计史，不在原地改写。
