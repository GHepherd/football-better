...
- 关键的是，reviewer 被**显式授权修订其他 agent 记忆条目中的「证据/置信」部分** —— 这正是用户今天在手工做的事（给 `extreme-mismatch-ucl` 加「n=2 均失败，降级为待验证」）。授权范围严格限定为：
  ...
  - **不可以**：改写原始论断本身（无论是否留痕）。若证据已足以推翻论断，正确做法是**新写一条记忆并链接旧条目**（`[[旧条目名]]`），保留推理痕迹
...
11. reviewer 对一个已存在的 analyzer 记忆条目追加了证据标注；原始论断文字未被改写
12. 样本量 n<5 时，结论明确标注「样本不足，仅作观察」

---

## 变更 2：方案必须落盘为文档，且永不覆盖（2026-09-11）

**背景。** 本规范原文把 `plan.md` 定为投注方案的产物（见「产物与命名」一节），但有两个洞：

1. `lottery-strategist` 的 manifest **没有落盘义务** —— 它的 `Output Format Requirements` 只规定「提供」什么，方案能否留下来取决于主会话在 dispatch 时有没有临时交代。这与本设计「产物走文件、不走上下文」的核心机制相悖：一旦主会话忘了给路径，方案就只存在于对话里，随会话消失。
2. `plan.md` 是**单一文件名**。同一比赛日出第二轮方案时，会覆盖第一轮的记录。方案是真实支出的记录，也是 reviewer 事后要逐份评分的对象，覆盖即销毁审计链——这与 §5.3.3 禁止改写原始论断是同一条原则。

**变更。**

- 产物命名改为 `matches/<本轮最早比赛日>/plan-NN.md`。写入前先 `ls` 该目录已有的 `plan-*.md`，取下一个可用序号（无则 `plan-01.md`）。
- `lottery-strategist` manifest 增加**强制落盘**条款，并规定文档骨架（比赛清单 / 注单表 / 总投入 ≤ 20 元 / 最大可能回报 / 风险提示 / 数据来源）。回执只返回路径 + 3–5 行摘要。
- **结论为观望（0 注）时同样写文件**：「不投」也是方案，也是评分对象。
- reviewer 对**每一份** `plan-NN.md` 分别评分，不合并计算。

**未变更。** 第 4 步的硬闸门（每次都要显式征求同意）不变；本变更只关乎「方案一旦产生就必须落盘」，不放松任何一道闸门。

**对照原验收标准。** 第 2、3、5 条（第 3 步停住、回答「要」后产出方案、说「只要分析」则不产出）语义不变，仅产物名由 `plan.md` 变为 `plan-NN.md`。
