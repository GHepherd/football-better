...
This is a **Claude Code agent-configuration repository**, not a software package. It contains no build system, test suite, package manager files, or deployable code. The only source files are agent manifests under `.claude/agents/`.
...
Requests in this domain are handled by a three-stage pipeline:
...
3. **Plan** — If the user wants a lottery ticket, launch `lottery-strategist` using the analyzer's probabilities to generate a ≤20 yuan 竞彩足球 plan.

Agents should be invoked in order; the strategist depends on analyzer output and the analyzer depends on gatherer output.
